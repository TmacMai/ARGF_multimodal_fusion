...
from utils import total, load_iemocap6, load_iemocap6_context
...
def main(options):
    ...
    train_set, valid_set, test_set, input_dims = load_iemocap6_context(data_path)
   ...
   #     pixelwise_loss = torch.nn.KLDivLoss().cuda()    #bad
        ...
        for e in range(epochs):
           ...
           #     ade = torch.cat([a_en,z],1)
            ...
            #    lde = torch.cat([l_en,z],1)
                ...
                g_loss = alpha * (adversarial_loss(discriminator(l_en), valid)+adversarial_loss(discriminator(v_en), valid)) +(1-alpha) * (rl1)
             ...
             #   v_c = classifier_3(fusion)
                ...
                v = classifier_3(v_en)
                l = classifier_3(l_en)
                c_loss = criterion(a,y)+criterion(v,y)+criterion(l,y)
	    ...
	    print ("Accuracy: ", accuracy_score(true_label, predicted_label))
            ...
            if bi_acc>best_acc:
               ...
               best_f1 = f1
               best_setting = current_setting
               np.save('iemocap_fusion.npy',fusion.cpu().data.numpy())
               np.save('iemocap_label_one_hot.npy',y)
               np.save('iemocap_weights.npy',weights.cpu().data.numpy())
               np.save('iemocap_predicted.npy',output_test)
...
if __name__ == "__main__":
    ...
    OPTIONS.add_argument('--output_dim', dest='output_dim', type=int, default=6)
