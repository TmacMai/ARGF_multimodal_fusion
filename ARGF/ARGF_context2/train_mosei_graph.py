...
from utils import total, load_mosei, load_mosei_context
...
def main(options):
    ...
    train_set, valid_set, test_set, input_dims = load_mosei_context(data_path)
...
#    params['text_dropout'] = [0, 0.1,  0.2, 0.3, 0.5,0.6,0.7,0.8]
  ...
  #  for i in range(total_settings):
   ...
   #     pixelwise_loss = torch.nn.KLDivLoss().cuda()    #bad
        ...
        optimizer_E = torch.optim.Adam(itertools.chain(encoder_a.parameters(), encoder_v.parameters(),encoder_l.parameters(),classifier_3.parameters()), lr=lr, betas=(b1, b2), weight_decay=decay)
        ...
        for e in range(epochs):
            ...
            for batch in train_iterator:
                ...
                rl1 = pixelwise_loss(a_de, x_a)+pixelwise_loss(v_de, x_v)+pixelwise_loss(l_de, x_t)
                ...
                g_loss =    alpha * (adversarial_loss(discriminator(a_en), valid) + adversarial_loss(discriminator(v_en), valid) )+(1-alpha) * (rl1)
                ...
                real_loss = adversarial_loss(discriminator(l_en), valid)
                avg_v += torch.sum(discriminator(v_en).squeeze().data)/ (len(train_set)*batch_sz)

                fake_loss = adversarial_loss(discriminator(v_en), fake) + adversarial_loss(discriminator(a_en), fake)
        ...
        if complete:
         ...
         #   best_model.eval()
            ...
            if bi_acc>best_acc:
               ...
               classes = classification_report(true_label, predicted_label,digits=4)
...
if __name__ == "__main__":
    ...
    OPTIONS.add_argument('--run_id', dest='run_id', type=int, default=2)
    OPTIONS.add_argument('--epochs', dest='epochs', type=int, default=500)
    OPTIONS.add_argument('--patience', dest='patience', type=int, default=20)
    OPTIONS.add_argument('--output_dim', dest='output_dim', type=int, default=3)
    OPTIONS.add_argument('--signiture', dest='signiture', type=str, default='mosei')     
